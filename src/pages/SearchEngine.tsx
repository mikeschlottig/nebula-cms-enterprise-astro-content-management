    ...
    <AppLayout container={false} className="bg-[#0B0F1A] min-h-[calc(100vh-64px)]">
      <div className="max-w-5xl mx-auto px-4 py-12 space-y-8">
        <div className="text-center space-y-4">
          <div className="inline-flex items-center justify-center p-3 rounded-2xl bg-primary/20 text-primary mb-2">
        ...
        <div className="relative max-w-2xl mx-auto">
          ...
          <Input 
            ...
            className="h-16 pl-12 pr-4 bg-white/5 border-white/10 rounded-2xl text-lg focus:ring-primary focus:border-primary placeholder:text-slate-600 shadow-2xl"
          />
          <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-1 text-[10px] text-slate-500 bg-white/5 px-2 py-1 rounded border border-white/10">
            <Terminal className="h-3 w-3" /> CMD+K
          </div>
        </div>
        <div className="flex justify-center gap-2">
          {['all', 'astro', 'cloudflare'].map((tab) => (
            <Button
              ...
              className={`capitalize rounded-full px-6 ${activeTab === tab ? 'bg-primary' : 'text-slate-400'}`}
            >
              {tab}
            </Button>
          ))}
          ...
          {isLoading ? (
            ...
            <AnimatePresence mode="popLayout">
              {filteredResults?.map((result, i) => (
                ...
                >
                  <Card className="glass-panel border-none hover:border-primary/30 transition-all cursor-pointer group">
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between">
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            <Badge variant="outline" className={`border-none ${result.category === 'Astro' ? 'bg-orange-500/10 text-orange-500' : 'bg-blue-500/10 text-blue-500'}`}>
                              {result.category}
                            </Badge>
                            <span className="text-[10px] text-slate-500 font-mono">Similarity: {(result.score * 100).toFixed(1)}%</span>
                          </div>
